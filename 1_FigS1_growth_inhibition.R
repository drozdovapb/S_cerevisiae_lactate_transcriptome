...
dat <- read.xlsx("LA_yeast_growth.xlsx") ## rename S...
...
ggplot(dat, aes(x=condition, y=Log2.Growth, col = condition)) + 
  ...
  theme_bw() + 
  scale_x_discrete(labels = c("control", "50 uM DLA", "500 uM DLA", "5 mM DLA", "45 mM DLA", "45 mM DLA")) + 
...
ggsave("growth_inhibition.png", width = 20, height = 8, units="cm")
